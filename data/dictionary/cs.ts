...
→ 핵심은 데이터를 필요한 순간 신뢰할 수 있는 형태로 제공하는 구조 설계.`,
      ...
      },
      {
        title: "💡 CAP 이론",
        summary:
          "분산 시스템에서 Consistency(일관성), Availability(가용성), Partition Tolerance(분할 내성) 세 가지 특성 중 동시에 모두 충족할 수 없고, 최대 2가지만 만족할 수 있다는 이론",
        date: "25.09.22",
        detail: `**💡 정의:**
**C (Consistency, 일관성)**: 모든 노드가 같은 시점에 동일한 데이터를 보여줌
**A (Availability, 가용성)**: 요청이 항상 실패 없이 응답을 반환
**P (Partition Tolerance, 분할 내성)**: 네트워크가 분리되더라도 시스템이 동작

**💡 왜 동시에 3가지는 불가능할까?**
네트워크가 분리(Partition 발생)되면, 노드 간 통신이 단절됨.

**C(일관성)을 지키려면** → 모든 노드가 같은 데이터가 될 때까지 응답을 지연하거나 거부해야 함 → A(가용성) 포기
**A(가용성)을 지키려면** → 네트워크가 끊겨도 각 노드가 독립적으로 응답해야 함 → 서로 다른 값이 생겨 C(일관성) 포기
**P(분할 내성)은** 분산 시스템 특성상 필수적이므로 → 결국 C vs A 중 하나를 포기해야 함

**💡 비유:**
여러 지점(노드)에 분산된 ATM 기기
- **일관성(C)**: 모든 지점에서 동시에 동일한 잔액 조회
- **가용성(A)**: 어느 지점에서든 인출 가능
- **분할 내성(P)**: 본점 서버와 일부 지점 연결이 끊겨도 운영 가능

→ 네트워크 단절 시, 잔액을 정확히 맞추려면 인출 불가(가용성 포기) / 인출을 허용하면 잔액 불일치(일관성 포기)

**💡 활용 예시:**
**CA (Consistency + Availability)** → Partition 발생 시 보장 불가
예: 전통적 관계형 DB (네트워크 안정 전제)

**CP (Consistency + Partition Tolerance)** → 가용성 일부 포기
예: HBase, MongoDB (Strong Consistency 모드)

**AP (Availability + Partition Tolerance)** → 일관성 일부 포기
예: Cassandra, DynamoDB

**💡 정리:**
CAP 이론은 네트워크 장애 상황(Partition)을 고려했을 때, 정확성(C) vs 가용성(A) 중 무엇을 포기할지 선택하는 문제임`,
        codeExample: ``,
      },
    ],
  },
