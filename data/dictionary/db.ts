export const dbTechnologies = [
  {
    ...
    details: [
        ...
        detail: `
          ...
          **💬 꼬리질문 2**
            **Q. MyISAM 과 MEMORY 등 다른 스토리지 엔진에 대해서도 설명해주세요.**
            ...
            ARCHIVE 엔진은 대량의 데이터를 압축하여 저장하는 스토리지 엔진입니다. 데이터가 Insert될 때마다 zlib로 압축되므로 디스크 I/O를 최소화하며, Insert 작업을 빠르게 처리할 수 있습니다. ARCHIVE 엔진은 Insert, Replace, Select 쿼리는 지원하지만 Delete와 Update는 지원하지 않습니다. 잠금 단위는 Row 수준으로 작동합니다. 주로 로그성 데이터나 대량 데이터에 저장하고 읽는 용도로 사용됩니다. ARCHIVE 엔진은 인덱스를 지원하지 않습니다.
      ...
      {
        title: "Q. MVCC(Multi-Version Concurrency Control) 원리와 구성 요소 대해 설명하세요.",
        date: "2025.10.07",
        detail: `
          MVCC는 데이터베이스에서 높은 동시성을 유지하면서 트랜잭션 격리와 데이터 무결성을 보장하기 위한 메커니즘입니다. MVCC를 구현하기 위해 주로 사용되는 구성 요소는 Undo Log, Redo Log, Buffer Pool입니다.

          Undo Log는 트랜잭션 실행 이전의 데이터를 기록하여 롤백이나 MVCC 기능을 구현하는 데 사용됩니다. 예를 들어, 사용자가 특정 데이터를 수정하려 할 때 Undo Log에 기존 데이터를 기록하면, 트랜잭션을 롤백하거나 다른 트랜잭션에서 이전 버전을 참조할 수 있습니다.

          Redo Log는 트랜잭션이 커밋된 후 시스템 장애가 발생할 경우, 변경 내용을 재적용하여 데이터의 무결성과 지속성을 보장합니다. 이를 통해 마지막 커밋 상태로 데이터베이스를 복원할 수 있습니다.

          Buffer Pool은 메모리 내 데이터를 관리하여 디스크 I/O를 최소화하고 성능을 향상시킵니다. Undo Log와 Redo Log의 동작도 Buffer Pool과 협력하여 수행되며, 데이터베이스의 빈번한 읽기/쓰기 작업이 메모리에서 처리되도록 지원합니다.

          이 세 가지 구성 요소가 함께 동작하면서 트랜잭션 격리와 데이터 무결성을 유지합니다. 예를 들어, 사용자가 데이터를 읽을 때 Undo Log를 통해 이전 버전을 참조하여 읽기 일관성을 확보하고, 트랜잭션 종료 시 Redo Log와 Buffer Pool이 협력하여 데이터를 지속적으로 저장하고 복구를 지원합니다.

          이러한 MVCC 는 읽기와 쓰기 작업 간 충돌을 최소화하여 높은 동시성을 제공하고, 데이터 읽기 시 락 경합이 없어 성능이 향상됩니다. 그리고 Dirty Read와 Non-Repeatable Read 문제를 방지할 수 있다는 장점이 있습니다. 하지만, 추가 저장 공간이 필요하며, 데이터 버전 관리가 복잡해질 수 있고, Phantom Read는 Serializable 격리 수준에서만 방지가 가능하다는 단점이 있습니다.

          **💬 꼬리질문 1**
            **Q. MVCC 의 동작 방식에 대해 설명해주세요.**

            첫 번째로, 스냅샷.. 즉, 트랜잭션 시작 시점의 데이터 버전을 읽으며, 다른 트랜잭션의 변경 작업은 현재 트랜잭션에 영향을 주지 않습니다.
            다음으로는 쓰기 작업을 진행합니다. 기존 데이터를 직접 수정하지 않고 새로운 버전을 생성하며, 커밋되지 않은 쓰기 작업은 다른 트랜잭션에서 보이지 않습니다.
            마지막으로 가비지 수집을 하여, 참조되지 않는 오래된 데이터 버전을 주기적으로 삭제하여 디스크 공간을 관리합니다.

          **💬 꼬리질문 2**
            **Q. MVCC 에서 발생할 수 있는 문제와 대응방안을 설명해주세요.**

            **첫 번째로,** Snapshot Anomaly 가 발생할 수 있습니다. 이는 랜잭션이 오래된 스냅샷을 참조하여 최신 데이터와 불일치가 발생할 수 있습니다. 이를 해결하려면 Serializable 격리 수준을 사용하거나 애플리케이션 레벨에서 데이터 동기화를 처리해야 합니다.
            **두 번째로는** Garbage Data 가 증가하는 문제가 생길 수 있습니다. MVCC는 여러 데이터 버전을 유지하기 때문에, 참조되지 않는 오래된 데이터가 쌓이면 디스크 공간을 낭비하고 성능을 저하시킬 수 있습니다. 이를 방지하기 위해 PostgreSQL의 VACUUM이나 MySQL InnoDB의 Purge Thread와 같은 가비지 수집 메커니즘을 사용합니다.
            **마지막 세 번째로는,** Lost Update 문제가 발생 할 수 있습니다. 이는 두 트랜잭션이 동시에 동일 데이터를 수정하면 변경 사항이 일부 손실될 수 있습니다. 이를 방지하려면 애플리케이션 레벨에서 낙관적 락(Optimistic Lock)이나 비관적 락(Pessimistic Lock)을 활용하고, MySQL에서는 Repeatable Read 격리 수준을 사용하여 예방할 수 있습니다.
        `
      },
      {
        title: "Q. Clustered Index vs Non-Clustered Index",
        date: "2025.10.09",
        detail: `
          클러스터드 인덱스와 논클러스터드 인덱스의 차이는 데이터가 인덱스와 어떻게 연결되어 저장되느냐에 있습니다.
          InnoDB에서는 **기본 키(Primary Key)**가 자동으로 클러스터드 인덱스로 설정됩니다. 클러스터드 인덱스는 데이터 자체가 인덱스의 리프 노드에 함께 저장되는 구조로, 테이블의 모든 데이터가 기본 키 순서대로 물리적으로 정렬되어 저장됩니다.
          예를 들어 users 테이블에서 id가 PK라면, 데이터는 id 순서대로 정렬되고,\`SELECT * FROM users WHERE id = 10;\` 같은 쿼리는
인덱스 탐색만으로 바로 데이터에 접근할 수 있습니다. 즉, 인덱스와 데이터가 일체화된 구조라서 조회가 매우 빠릅니다. 다만, 데이터 정렬 기준이 하나뿐이므로 클러스터드 인덱스는 테이블당 하나만 존재할 수 있습니다.
반면 논클러스터드 인덱스, 즉 **세컨더리 인덱스(Secondary Index)**는 데이터와 인덱스가 분리된 구조입니다. 리프 노드에는 실제 데이터가 아닌 기본 키 값이 저장되어 있어서, 검색 시 먼저 세컨더리 인덱스에서 PK 값을 찾고, 그 PK를 이용해 클러스터드 인덱스를 다시 조회하는 Double Lookup 과정이 발생합니다.
예를 들어 email 컬럼에 인덱스가 있을 때, \`SELECT * FROM users WHERE email='test@example.com';\` 쿼리는 먼저 email 인덱스에서 PK를 찾고, 그 후 PK로 실제 데이터를 가져옵니다. 이 구조 때문에 세컨더리 인덱스는 여러 개 생성 가능하지만, 조회 시 추가적인 I/O가 발생합니다.
그리고 번외로 MySQL에서는 Primary Key는 하나만 존재할 수 있습니다. 하지만 여러 컬럼을 묶은 **복합 기본 키(Composite Primary Key)**는 만들 수 있습니다.
예를 들어 orders 테이블에서 (user_id, product_id)를 PK로 설정하면, 이 두 컬럼이 결합되어 하나의 클러스터드 인덱스 키가 됩니다. 따라서 데이터는 (user_id, product_id) 순서로 물리적으로 정렬되며, 세컨더리 인덱스가 있을 경우 그 리프 노드에는 해당 인덱스 키와 함께 복합 PK 전체가 저장되어 실제 데이터를 찾게 됩니다.
(메모리에 캐시되어 있으면 버퍼 풀을 통해 접근하지만, 원본은 항상 디스크에 존재합니다)
        `
      }
    ]
  },
