...
그러던 중, 오픈소스 기여 방법을 안내해주는 커뮤니티를 알게 되었고, 그곳에서 멘토링 프로그램에 참여하게 되었습니다. 이슈를 찾고, 기여할 부분을 고민하며, 본격적으로 오픈소스의 세계에 발을 들이게 되었습니다.

Kafka를 공부하던 어느 날, 문득 Spring Kafka의 내부 구현이 궁금해졌고, 자연스럽게 GitHub 저장소를 들여다보게 되었습니다. 거기서 발견한 하나의 이슈 **— 작아 보이지만 사용자 혼란을 야기할 수 있는 문제 —** 를 계기로 생애 첫 Pull Request를 만들게 되���고, 운 좋게도 머지되며 **Spring Kafka의 공식 Contributor**가 되었다는 게 정말 뿌듯했습니다.
...
하지만 실제 \`@RetryableTopic\`의 JavaDoc을 확인해보니 다른 내용이 적혀있었습니다.
...
먼저 가장 명확한 부분부터 수정하기로 했습니다. \`@RetryableTopic\` 어노테이션 클래스의 JavaDoc을 공식 문서와 실제 구현에 맞게 수정하는 것이었습니다.
...
변경 사항은 단순해 보이지만, 이런 작은 불일치가 개발자들에게 미치는 영향을 생각하면 결코 사소한 수정이 아니라고 생각했습니다. 특히 JavaDoc은 IDE에서 자동완성이나 도움말을 볼 때 가장 먼저 참조하는 정보이기 때문에 정확해야 합니다.
...
JavaDoc 수정만으로는 부족하다고 생각했어요. 테스트 코드에서도 같은 문제가 있었습니다. \`RetryTopicConfigurationProviderTests\`라는 테스트 클래스를 살펴보니, 빈 이름을 하드코딩된 문자열로 직접 명시하고 있었습니다.
...
이 수정이 중요한 이유는 두 가지였습니다:

1. **일관성 확보**: 하드코딩된 문자열 대신 상수를 사용함으로써 코드 전체의 일관성을 높였습니다.
2. **유지보수성 향상**: 만약 나중에 빈 이름이 변경되더라도 상수 하나만 수정하면 되니까 훨씬 안전합니다.

사실 이런 리팩터링은 오픈소스 기여에서 정말 중요한 부분이라고 생각합니다. 단순히 버그만 고치는 게 아니라, 코드 품질 자체를 개선하는 거니까요.
...
코드 수정을 마치고 나서 가장 긴장되는 순간이 왔습니다. 바로 Pull Request를 작성하는 것이었습니다. 첫 오픈소스 기여라서 정말 떨렸습니다.
...
PR을 올리고 나서 며칠 후, Spring Kafka의 메인테이너 중 한 분이 리뷰를 남겨주셨습니다. 다���히 긍정적인 피드백이었고, 몇 가지 작은 수정 요청이 있었습니다:

1. **커밋 메시지 형식**: Spring 프로젝트의 커밋 메시지 컨벤션에 맞게 조정
2. **테스트 케이스 추가**: 변경사항이 올바르게 동작하는지 확인하는 테스트 추가

이런 피드백을 받으면서 오픈소스 프로젝트마다 고유한 문화와 규칙이 있다는 걸 실감했습니다. 그리고 메인테이너들이 얼마나 많은 노력을 기울이는지 알 수 있었습니다.
...
사실 Spring Kafka 기여 전에 한 번 실패한 경험이 있었습니다. **Apache Jackrabbit Oak** 프로젝트에서 시도했던 기여가 머지되지 못했습니다. 하지만 이 실패 경험이 오히려 더 큰 배움을 주었다고 생각합니다.

<br>

### 💡 Apache Jackrabbit Oak에 대한 이해

**Apache Jackrabbit Oak**는 처음 들어보는 분들이 많을 텐데, 간단히 설명하면 파일이나 문서 같은 데이터를 트리 구조로 저장하고 관리해주는 시스템입니다. Java로 만들어졌고, Adobe Experience Manager(AEM) 같은 유명한 콘텐츠 관리 시스템에서 내부 저장소로 사용되고 있습니다.

예를 들어, 웹사이트에서 페이지를 ���들거나 이미지를 업로드할 때 그 데이터를 저장하는 데 Oak가 쓰일 수 있습니다. 파일이나 폴더를 디렉토리처럼 관리하고, 누가 언제 바꿨는지 기록도 남길 수 있어서 문서 관리 시스템이나 CMS에 딱 맞는 구조입니다.

처음에는 이런 복잡한 시스템에 기여할 수 있을까 싶었는데, 다행히 비교적 간단한 이슈를 찾을 수 있었습니다.

<br>

### 발견한 이슈와 해결 시도

당시 발견한 이슈는 코드 중복 제거에 관한 것이었습니다:
...
\`MAX_SEGMENT_SIZE\` 상수가 **Segment** 클래스와 **SegmentDataUtils** 클래스에 중복으로 정의되어 있었습니다. 같은 값(\`1 << 18\`)을 두 곳에서 따로 정의하고 있었던 겁니다.

이런 중복은 유���보수 관점에서 좋지 않습니다. 나중에 값을 변경해야 할 때 두 곳을 모두 수정해야 하고, 실수로 한 곳만 바꾸면 버그가 생길 수 있습니다.

해결 방법은 간단했어요:
1. **Segment** 클래스의 \`MAX_SEGMENT_SIZE\`에 \`public\` 접근 제어자 추가
2. **SegmentDataUtils**에서 중복 정의 제거하고 \`Segment.MAX_SEGMENT_SIZE\` 참조

정말 간단한 수정이라서 자신 있게 PR을 올렸습니다.
...
하지만 결과는 **머지 실패**였습니다. 솔직히 좀 당황스러웠습니다.
...
실패 이유는 생각보다 단순했습니다. **이미 저보다 먼저 해당 이슈를 캐치하고 PR을 올린 사람이 있었기 때문**이었습니다.

메인테이너가 남긴 코멘트를 보니, 제가 PR을 올리기 몇 시간 전에 다른 기여자가 같은 내용의 PR을 이미 올렸던겁니다. 그리고 그 PR이 먼저 리뷰되고 머지된 거였습니다.

처음에는 정말 억울했습니다. "내가 먼저 발견했는데!"라는 생각도 들었습니다. 하지만 시간이 지나면서 이 경험이 오히려 소중한 교훈을 준다는 걸 깨달았습니다.

<br>

### 실패에서 얻은 소중한 교훈들

이 실패 경험을 통해 오픈소스 생태계에서 초보자가 놓치기 쉬운 중요한 점들을 배웠습니다:

#### 1. 이슈 상태 확인의 중요성

오픈소스 프로젝트에서는 이슈가 해결되었어도 바로 Close되지 않는 경우가 많습니다. PR이 머지된 후에야 이슈가 닫히는 경우도 있고, 때로는 메인테이너가 깜빡하고 이슈를 열어둔 채로 두는 경우도 있습니다.
...
오픈소스 커뮤니티에는 암묵적인 룰이 있습니다. 누군가 해당 이슈를 해결하겠다고 **Assign을 받았거나 댓글로 의사를 밝혔다면**, 그걸 존중해야 한다는 겁니다.

물론 법적으로 강제되는 건 아닙니다.ㅎㅎ 하지만 커뮤니티의 건전한 문화를 위해서는 이런 에티켓을 지키는 게 중요하다고 생각합니다. 만약 누군가 이미 작업 중이라면, 다른 이슈를 찾거나 해당 작업이 오랫동안 진행되지 않을 때 정중하게 문의하는 게 좋다고 봅니다.

#### 3. 올바른 오픈소스 기여 프로세스

이 경험을 통해 올바른 기여 프로세스를 배웠습니다:
...
처음에는 이 실패가 정말 아쉬웠지만, 지금 생각해보면 오히려 좋은 경험이었습니다. 만약 이 PR이 성공했다면, 오픈소스 기여가 쉽다고 착각했을 수도 있었을겁니다.

실패를 통해 오픈소스 커뮤니티의 문화와 룰을 배웠고, 더 신중하고 체계적으로 접근하는 방법을 익혔습니다. 그리고 이런 경험이 있었기 때문에 Spring Kafka 기여에서는 더 꼼꼼하게 준비할 수 있었습니다.

<br>

## 🎉 2.7. 기여 성공 후의 변화

Spring Kafka에 성공적으로 기여한 후, 개발자로서 여러 가지 변화를 경험했습니다.

<br>

### 자신감 향상

가장 큰 변화는 자신감입니다. "나도 오픈소스에 기여할 수 있구나"라는 확신이 생겼습니다. 처음에는 "내가 이런 큰 프로젝트에 기여할 수 있을까?"라는 의구심이 있었는데, 실제로 해보니 생각보다 어렵지 않았던거 같습니다

물론 쉽다는 건 아닙니다. 하지만 체계적으로 접근하고 꾸준히 노력하면 충분히 가능하다는 걸 깨달았습니다.

<br>

### 코드 품질에 대한 인식 변화

오픈소스 기여를 하면서 코드 품질에 대한 기준이 높아졌습니다. 메인테이너들의 꼼꼼한 리뷰를 받으면서, 단순히 동작하는 코드가 아니라 **읽기 쉽고 유지보수하기 좋은 코드**의 중요성을 실감하게 되었습니다.

이제 회사에서 코드를 작성할 때도 다음과 같은 점들을 더 신경 쓰게 되었습니다:
- 명확하고 일관된 네이밍
- 적절한 주석과 문서화
- 테스트 코드의 중요성
- 코드 리뷰의 가치
...
오픈소스 프로젝트가 어떻게 운영되는지, 메인테이너들이 얼마나 많은 노력을 기울이는지 알게 되었습니다. 그리고 전 세계 개발자들이 어떻게 협업하는지도 직접 경험할 수 있었습니다.

이런 경험을 통해 오픈소스에 대한 감사함도 커졌습니다. 우리가 당연하게 사용하는 수많은 라이브러리와 프레임워크들이 모두 누군가의 무료 기여로 만들어진 거잖아요.ㅎㅎ

<br>

# ✏️ 3. 결론

이번 Spring Kafka 기여를 통해 **문서와 코드의 ���일치 문제를 해결**하며, 보다 명확하고 유지보수하기 쉬운 기반을 마련하는 데 작은 보탬이 될 수 있었습니다.
...
오픈소스 기여를 어렵게만 느끼는 분들도 많을 텐데, 제 경험을 통해 말씀드리고 싶은 건 **완벽하게 아는 상태에서 시작하는 사람은 없다**는 겁니다. 저 역시 문서 하나 고치고, 테스트 코드 한 줄 바꾸는 데서 시작했습니다.

**작은 기여도 충분히 의미 있고**, 오픈소스 커뮤니티는 그런 기여를 소중히 여깁니다. 중요한 건 거창한 기능을 추가하는 게 아니라, 문제를 발견하고 해결하려는 마음가짐이라고 생각합니다.

<br>

### 첫 기여를 위한 실전 조언

제 경험을 바탕으로 몇 가지 실전 조언을 드리고 싶습니다.:
...
오픈소스는 전 세계 개발자들이 함께 만들어가는 공동체예요. 여러분의 작은 기여 하나하나가 모여 더 나은 소프트웨어 생태계를 만들어가는 겁니다.

**첫 번째 PR을 두려워하지 마세요. 모든 전문가도 처음에는 초보자였으니까요.**

저도 이제 막 시작한 단계지만, 앞으로도 꾸준히 오픈소스에 기여하면서 개발자로서 성장해나가고 싶어요. 그리고 언젠가는 다른 초보자들에게 도움을 줄 수 있는 멘토가 되고 싶습니다.

여러분도 용기를 내서 첫 번째 기여를 시작해보세요. 분명 저처럼 뿌듯하고 의미 있는 경험을 하실 수 있을 거라고 생각합니다! 🚀
`
