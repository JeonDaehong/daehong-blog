...
문제��� 여기서 시작됩니다. 만약 개발자가 실수로 \`WHERE year = 2024\` 조건을 빼먹고 다음과 같이 쿼리를 작성한다면
...
이 방식의 장점은 쓰기 부담이 적다는 점입니다. 대부분의 작업이 Append 중심이므로 쓰기 성능이 뛰어납니다. 그러나 읽기 과정에서 병합 연산이 추가되므로 쿼리 성능이 저하될 수 있습니다. 특히 Delete File이 누적되면 이 문제가 더욱 심각해질 수 있어, 주기적으로 Compaction 작업을 ���행해 병합된 파일을 만들어주는 관리가 필요합니다.  
